...
def  execute_nmon(srv_list):
    for srv in srv_list:
        srv['srv'].execute('cd ~; mkdir -p ~/nmon')
        which_nmon = srv['srv'].execute('. ~/.bash_profile; which nmon')[0].split('\n')[0]
        if which_nmon:
            srv['srv'].execute('ps -ef|grep nmon|grep 180|awk \'{print $2}\'|xargs  kill -9')
            srv['srv'].execute('. ~/.bash_profile;cd ~/nmon; nmon -f -s 5 -c 180')
            print '%s finished!!'%srv['host']
        else:
            print '%s nmon not exists!!'%srv['host']
    

def get_nmon_file(srv_list):
    for srv in srv_list:
        nmon_file = srv['srv'].execute('cd ~/nmon; ls -t *.nmon|head -1')[0].split('\n')[0]
        srv['nmon_file'] = nmon_file
        
        currdir =  srv['srv'].execute('cd ~/nmon; pwd')[0].split('\n')[0]
        srv['srv'].get('%s/%s' % (currdir,srv['nmon_file']), 'c:\\%s' % srv['nmon_file'])
        print '%s get nmon file success!!'%srv['host']
        
def disconn_hosts():
    for srv in srv_list:
        srv['srv'].close()
        print '%s disconned!!'%srv['host']

if __name__ == '__main__':
    
    srv_list = conn_hosts()  
    #execute_nmon(srv_list)
    get_nmon_file(srv_list)
    disconn_hosts()
